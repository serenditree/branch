...
import io.serenditree.fence.model.FenceHeaders;
 ...
 * requested resource method. Security information is added to the {@link io.serenditree.fence.model.FenceContext}.
 */
...
public class FenceFilter implements ContainerRequestFilter {
    ...
    private final Provider<Event<io.serenditree.fence.model.FenceContext>> authenticationEvent;
    private final Provider<Tracer> fenceTracer;
    ...
    public FenceFilter(
        ...
        Provider<Tracer> fenceTracer,
        @FenceContext Provider<Event<io.serenditree.fence.model.FenceContext>> authenticationEvent,
        ...
        if (StringUtils.isNotBlank(notAuthenticatedUser.getToken())) {
            ...
            if (authenticatedUser != null) {
                ...
                this.authenticationEvent
                    .get()
                    .fire(new io.serenditree.fence.model.FenceContext(authenticatedUser, true));
            } else {
                this.abortWith(
                    containerRequestContext,
     ...
     * to an implementation of {@link AuthorizationService} and if the authorization fails the request is aborted.
     *
     * @param containerRequestContext {@link ContainerRequestContext}
    ...
    /**
     * Uses an implementation {@link AuthenticationService} for the registration of new users and adds the created
     * {@link FencePrincipal} to the {@link io.serenditree.fence.model.FenceContext}.
     *
     * @param containerRequestContext {@link ContainerRequestContext}
    ...
    private void signUp(
        final ContainerRequestContext containerRequestContext,
        final FencePrincipal clientUser
    ) {
        try {
            FencePrincipal persistenceUser = this.authenticationService.get().signUp(clientUser);
            this.authenticationEvent.get().fire(new io.serenditree.fence.model.FenceContext(persistenceUser, true));
    ...
    /**
     * Uses an implementation {@link AuthenticationService} to check the authenticity of clients and adds a verified
     * {@link FencePrincipal} with security token to the {@link io.serenditree.fence.model.FenceContext}. Identity is
     * checked by means of username
     * and password.
     *
     * @param containerRequestContext {@link ContainerRequestContext}
     * @param clientUser              {@link FencePrincipal} holding basic auth-relevant information.

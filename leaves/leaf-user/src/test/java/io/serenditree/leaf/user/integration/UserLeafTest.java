package io.serenditree.leaf.user.integration;
...
class UserLeafTest {
    ...
    @Order(2)
    void signUpInvalid(Headers headers, Status expectedStatus) {
        given()
            .headers(headers)
            .when()
            .post("sign-up")
    ...
    static Stream<Arguments> signUpHeadersSource() {
        return Stream.of(
            ...
            Arguments.of(
                new Headers(
                    new Header(FenceHeaders.USERNAME, UUID.randomUUID().toString()),
                    new Header(FenceHeaders.PASSWORD, "word-list")
                ),
                Status.BAD_REQUEST
            ),
            // Invalid email
            ...
            Arguments.of(
                new Headers(
                    new Header(FenceHeaders.USERNAME, UUID.randomUUID().toString()),
                    new Header(FenceHeaders.PASSWORD, PASSWORD),
                    new Header(FenceHeaders.EMAIL, "invalid@email")
                ),
                Status.BAD_REQUEST
            ),
            // Username too long.
    ...
    @Order(3)
    void signInWrongUsernameOrPassword(Headers headers, Status expectedStatus) {
        given()
            .headers(headers)
            .when()
            .post("sign-in")
    ...
    @Order(4)
    void retrieveByUsername(String username, Status expectedStatus) {
    ...
    @Order(5)
    void retrieveBySubstring(String substring, Status expectedStatus) {
        given()
            ...
            .body("$", expectedStatus == Status.OK ? iterableWithSize(1) : iterableWithSize(0));
    }
    ...
    @Order(6)
    void delete(Header header, Long id, Status expectedStatus) {
        given()
            .header(header)
            .pathParam("id", id)
            .pathParam("includeContributions", false)
            .when()
            .delete("{id}/{includeContributions}")
            .then()
            .statusCode(expectedStatus.getStatusCode());
    }

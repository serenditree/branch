package io.serenditree.root.test.authentication;
...
public class Authenticator {
    ...
    public static void cleanup(FencePrincipal principal) {
        try (Response response = CLIENT.target("http://localhost:8081/api/v1/user/{id}/{includeContributions}")
            .resolveTemplates(Map.of("id", principal.getId(), "includeContributions", false))
            .request()
